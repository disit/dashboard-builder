...
if(isset($_SESSION['loggedUsername']) && $_SESSION['loggedUsername']) 
{
    ...
    if(!checkSynopticId($link, $dashboardId, $accessToken, $ownershipApiBaseUrl, $synOwnElmtType)) {
        eventLog("invalid request for delSynopticDelegation.php for id = $dashboardId user: ".$_SESSION['loggedUsername']);
        exit;
    }
    
    $delegationId = $_REQUEST['delegationId'];
    if (checkVarType($delegationId, "integer") === false) {
        eventLog("Returned the following ERROR in delSynopticDelegation.php for id = ".$delegationId.": ".$delegationId." is not an integer as expected. Exit from script.");
        exit();
    }
