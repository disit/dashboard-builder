...
if(isset($_SESSION['loggedUsername']) && $_SESSION['loggedUsername']) 
{
    ...
    if (checkVarType($dashboardId, "integer") === false) {
        eventLog("Returned the following ERROR in delSynopticDelegation.php for id = ".$dashboardId.": ".$dashboardId." is not an integer as expected. Exit from script.");
        exit();
    }
    ...
    if(!checkSynopticTplId($link, $dashboardId, $accessToken, $ownershipApiBaseUrl, $synTplOwnElmtType)) {
        eventLog("invalid request for delSynopticTplDelegation.php for id = $dashboardId user: ".$_SESSION['loggedUsername']);
        exit;
    }
    
    $delegationId = $_REQUEST['delegationId'];
    if (checkVarType($delegationId, "integer") === false) {
        eventLog("Returned the following ERROR in delSynopticTplDelegation.php for id = ".$delegationId.": ".$delegationId." is not an integer as expected. Exit from script.");
        exit();
    }
