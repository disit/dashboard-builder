...
if(isset($_SESSION['loggedUsername']))
{
    ...
    if (checkVarType($dashboardId, "integer") === false) {
        eventLog("Returned the following ERROR in delSynopticGroupDelegation.php for id = ".$dashboardId.": ".$dashboardId." is not an integer as expected. Exit from script.");
        exit();
    }
    ...
    if(!checkSynopticId($link, $dashboardId, $accessToken, $ownershipApiBaseUrl, $synOwnElmtType)) {
        eventLog("invalid synoptic for id = $dashboardId user: ".$_SESSION['loggedUsername']);
        exit;
    }
	
    $delegationId = $_REQUEST['delegationId'];
    if (checkVarType($delegationId, "integer") === false) {
        eventLog("Returned the following ERROR in delSynopticGroupDelegation.php for delegationId = ".$delegationId.": ".$delegationId." is not an integer as expected. Exit from script.");
        exit();
    }
