...
class WidgetGisWFSFactory extends aGenericWidgetFactory
{
    ...
    function completeWidget()
    {
        ...
        */
        ...
        else
        {
            ...
            if($this->widgetRole == 'main')
            {
                ...
                switch($this->widgetTypeDbRow['icon'])
                {
                    case "servicemap.png":
                        ...
                        $this->startParams->link_w = "https://servicemap.disit.org/WebAppGrafo/api/v1/?selection=" . $this->selection . "&categories=" . $categories . "&maxResults=200&format=html";
                        break;
            ...
            {
                if($this->startParams->link_w == 'gisTarget')
                {
                    $this->startParams->parameters = "{\"latLng\":[" . $this->mapCenterLng . "," . $this->mapCenterLat . "],\"zoom\":" . $this->mapZoom . "}"; 
                }
                
                $this->startParams->id_metric = "GisWFS";
                $this->startParams->name_w = str_replace("ToBeReplacedByFactory", "GisWFS", $this->startParams->name_w); 
            }
        }
