...
if(isset($_SESSION['loggedUsername']) && $_SESSION['loggedUsername']) 
{
    ...
    if (checkVarType($dashboardId, "integer") === false) {
        eventLog("Returned the following ERROR in changeSynopticVisibility.php for id = ".$dashboardId.": ".$dashboardId." is not an integer as expected. Exit from script.");
        exit();
    };
    $dashboardTitle = mysqli_real_escape_string($link, $_REQUEST['synTitle']);
    ...
    {
        $q = "UPDATE DashboardWizard SET ownership='$dbVisibility' WHERE high_level_type = 'Synoptic' and id = '$dashboardId'";
        ...
        if($r) 
        {
            ...
            if($newVisibility == 'public')
            {
              ...
              //  $apiUrl = $personalDataApiBaseUrl . "/v1/apps/" . $dashboardId . "/delegations?accessToken=" . $accessToken . "&sourceRequest=dashboardmanager";
                ...
                $apiUrl = $personalDataApiBaseUrl . "/v3/apps/" . $dashboardId . "/delegations?accessToken=" . $accessToken . "&sourceRequest=$synMgtSrcReq&elementType=$synOwnElmtType";
                ...
                if(strpos($http_response_header[0], '200') !== false) 
                {
                    ...
                    $callBody = ["usernameDelegated" => "ANONYMOUS", "elementId" => $dashboardId, "elementType" => $synOwnElmtType];
