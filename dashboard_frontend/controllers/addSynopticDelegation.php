...
if(isset($_SESSION['loggedUsername']) && $_SESSION['loggedUsername']) {
    ...
    if (checkVarType($dashboardId, "integer") === false) {
        eventLog("Returned the following ERROR in addSynopticDelegation.php for id = ".$dashboardId.": ".$dashboardId." is not an integer as expected. Exit from script.");
        exit();
    }
    ...
    if($ds && $bind) {
        if(checkLdapRole($ds, $newDelegated, "RootAdmin", $ldapBaseDN)) {
			...
			if(isset($_SESSION['refreshToken'])) {
				...
				$callBody = ["usernameDelegated" => $newDelegated, "elementId" => $dashboardId, "elementType" => $synOwnElmtType];
