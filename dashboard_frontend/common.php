...
function checkLdapOrganizations($connection, $userDn, $baseDn)
{
    ...
    if(sizeof($orgsArray)==0) {
        if(isset($GLOBALS['defaultOrganization']))
            array_push($orgsArray, $GLOBALS['defaultOrganization']);
    }    
    return $orgsArray;
}
...
function get_access_token($token_endpoint, $username, $password, $client_id, $client_secret = ""){
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS,
        "username=".$username."&password=".$password."&scope=openid&grant_type=password&client_id=".$client_id.($client_secret ? "&client_secret=$client_secret" : ""));
