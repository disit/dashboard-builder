...
echo("*** Starting IOT_Last10min_Feed_DashboardWizard SCRIPT at: ".$start_time_ok."\n");
...
// if($rsIP) {
   ...
   //  while ($rowIP = mysqli_fetch_assoc($rsIP)) {
        ...
        $end_queryTime = new DateTime(null, new DateTimeZone('Europe/Rome'));
        $end_query_script_time = $end_queryTime->format('c');
        $end_query_script_time_string = explode("+", $end_query_script_time);
    ...
    /*    // Last 10 Minutes
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTPHEADER => array(
                'Cookie: PHPSESSID=a0f2sanfa97hg5583r28g79cr1'
            ),
        ));

        echo("\nSearching new IoT Devices created between: " . $start_query_script_time_string[0] . " and " . $end_query_script_time_string[0] . "\n\n");
...
echo $url."\n\n".$response."\n\n";
        ...
        foreach ($responseArr->data as $key2 => $val2) {

            $queryIP = "SELECT DISTINCT kbIP FROM Dashboard.Organizations WHERE broker = '" . $responseArr->data[$key2]->contextBroker . "';";
            $rsIP = mysqli_query($link, $queryIP);
            if($rsIP) {
                if ($rowIP = mysqli_fetch_assoc($rsIP)) {
                //    $organizations = $rowOrg['organizationName'];
               //     $kbUrl = $rowOrg['kbUrl'];
                    $kbHostIp = $rowIP['kbIP'];
                }
            ...
            $queryIotSensorDecoded = "select distinct ?n ?a ?avn ?avt ?dt ?u ?serviceType ?org ?imp ?brokerName ?model ?mobile ?lat ?lon { " .
                ...
                "OPTIONAL {<" . $responseArr->data[$key2]->uri . "> km4c:organization ?org.} " .
                "OPTIONAL {<" . $responseArr->data[$key2]->uri . "> <http://www.w3.org/2003/01/geo/wgs84_pos#lat> ?lat.} " .
                "OPTIONAL {<" . $responseArr->data[$key2]->uri . "> <http://www.w3.org/2003/01/geo/wgs84_pos#long> ?lon.} " .
                "OPTIONAL {<" . $responseArr->data[$key2]->uri . "> km4c:model ?model.} " .
            ...
            foreach ($resArray['results']['bindings'] as $key => $val) {
                ...
                if ($serviceChangeBuffer["current"] != $serviceChangeBuffer["last"]) {
                    if ($processId == 0) {
                        ...
                        $id = $open_search->checkDuplicateDocument(
                            $high_level_type,
                         $sub_nature, $instance_uri,
                        $get_instances, '', $unique_name_id, $open_search::default_index_name, '', '',true, '');
                ...
                if ($processId == 0) {
                    ...
                    $id = $open_search->checkDuplicateDocument($high_level_type, $sub_nature, 
                    $instance_uri,
...
foreach ($modelArray as $mod) {
    ...
    echo($totCount . " - IOT MODEL: " .$device_model_name_model . "\n");
    // $value_type_model
