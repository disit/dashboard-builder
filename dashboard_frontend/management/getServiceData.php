...
$list = array();
if (isset($_REQUEST['role_session_active'])) {
...
//if (($role_session_active == "RootAdmin") || ($role_session_active == "ToolAdmin") || ($role_session_active == "AreaManager")) {
    ...
    if ($type == 'From Dashboard to IOT Device') {
        ...
        while ($row0 = mysqli_fetch_assoc($result0)) {
            ...
            /***/
            $listFile0 = array("process_name_ST" => $row0['Process_ST'], "KB_Ip" => $row0['KB_Ip'], "Graph_Uri" => $graph_uri, "phoenix_table" => $row0['phoenix_table'], "process_path" => $row0['process_path'], "process_type" => $row0['process_type'], "licence" => $row0['licence'], "webpage" => $row0['webpage'], "telephone" => $row0['telephone'], "mail" => $row0['mail'], "owner" => $row0['responsible'], "Disces_Ip" => $row0['DISCES_Ip']);
        ...
        if ($result) {
            ...
            if ($total0 > 0) {
                while ($row0 = mysqli_fetch_assoc($result0)) {
                    $listFile0 = array("process_name_ST" => $row0['Process_name'], "KB_Ip" => $row0['KB_Ip'], "Graph_Uri" => $row0['Graph_Uri'], "phoenix_table" => $row0['phoenix_table'], "process_path" => $row0['process_path'], "process_type" => $row0['process_type'], "licence" => $row0['licence'], "webpage" => $row0['webpage'], "telephone" => $row0['telephone'], "mail" => $row0['mail'], "owner" => $row0['responsible'], "Disces_Ip" => $row0['DISCES_Ip']);
                    $process_path = $row0['process_path'];
                    array_push($process_list, $listFile0);
                }
            }
        ...
        if ($total0 > 0) {
            while ($row = $result0->fetch_assoc()) {
                ...
                if ($total0 > 0) {
                    while ($row0 = mysqli_fetch_assoc($result0)) {
                        $listFile0 = array("process_name_ST" => $row0['Process_name'], 
                                            "KB_Ip" => $row0['KB_Ip'], 
                                            "Graph_Uri" => $row0['Graph_Uri'], 
