...
<!-- Custom CSS -->
<link href="../css/dashboard.css?=<?php
                                    echo time();
                                    ?>" rel="stylesheet">
...
<style type="text/css">
    ...
    /*adding new color for delegation and group delegation*/
    .delegateDashBtn {
        height: 20px;
...
<body style="overflow-y: hidden !important">
                        ...
                        <div class="tab-content">
                            ...
                            <div id="delegationsCntGroup" class="tab-pane fade in">
                                <div class="row" id="delegationsFormRowGroup">
                                    <legend>
                                        <div class="col-xs-12 centerWithFlex modalFirstLbl" id="newDelegationLblGroup">
                                    ...
                                    <legend>
                                        <div class="col-xs-12 centerWithFlex" id="currentDelegationsLblGroup">
    ...
    <script type='text/javascript'>
                        ...
                        success: function(response) {
                            ...
                            if (response.code === '200') {
                                ...
                                var formattedData = validRecords.map(function(item) {
                                    ...
                                    //Naldi 09/07/2025 -> consider visibility contains MyOwn
                                    if (loggedRole === 'RootAdmin' || item.visibility.includes("MyOwn")){//|| item.visibility === 'MyOwnPublic' || item.visibility === 'MyOwnPrivate') {
                                    ...
                                    //Naldi 09/07/2025 -> consider visibility contains MyOwn
                                    if (loggedRole === 'RootAdmin' || item.visibility.includes("MyOwn") || item.visibility === 'public' || item.visibility === 'delegated'){//|| item.visibility === 'MyOwnPublic' || item.visibility === 'MyOwnPrivate' || item.visibility === 'public' || item.visibility === 'delegated') {
                                    ...
                                    }else if (item.visibility.includes('MyOwn')) { //Naldi 09/07/2025 -> consider visibility contains MyOwn
                                        management_button = '<button type="button" class="delegateDashBtn delegateGroup" onclick="func_management(\'' + item.filename + '\',\'' + item.deviceid + '\',\'' + item.contextbroker + '\',\'' + item.devicetype + '\',\'' + item.visibility + '\',\'' + item.k1 + '\',\'' + item.k2 + '\', \'' + item.description + '\', \'' + item.subnature + '\', \'' + item.filename + '\', \'' + item.language + '\', \'' + item.filesize + '\', \'' + item.filetype + '\', \'' + item.date + '\', \'' + item.latitude + '\', \'' + item.longitude + '\',\'' + item.newfileid + '\')">' + item.visibility + '</button>';
                                    } else if (loggedRole === 'RootAdmin') {  //Naldi 14/07/2025 -> let root admin change delegation 
                                        let classBtn = 'delegatedBtn';
                                        if(item.visibility === 'public')
                                            classBtn = "editDashBtn";
                                        else if(item.visibility === 'private')
                                            classBtn="delDashBtn";
                                        else if(item.visibility === 'delegated')
                                            classBtn="delegateDashBtn";
                                        else if(item.visibility.includes('delegated'))
                                            classBtn = "delegateDashBtn delegateGroup";
                                        management_button = '<button type="button" class="'+ classBtn +'" onclick="func_management(\'' + item.filename + '\',\'' + item.deviceid + '\',\'' + item.contextbroker + '\',\'' + item.devicetype + '\',\'' + item.visibility + '\',\'' + item.k1 + '\',\'' + item.k2 + '\', \'' + item.description + '\', \'' + item.subnature + '\', \'' + item.filename + '\', \'' + item.language + '\', \'' + item.filesize + '\', \'' + item.filetype + '\', \'' + item.date + '\', \'' + item.latitude + '\', \'' + item.longitude + '\',\'' + item.newfileid + '\')">' + item.visibility + '</button>';
        ...
        function func_management(name, deviceid, contextbroker, devicetype, visibility, k1, k2, description, subnature, filename, language, filesize, filetype, date, latitude, longitude, newfileid) {
            $("#delegationsModal").modal('show');
            $("#delegationHeadModalLabel").html("File - " + name);
            var newVisibility = '';
            if (visibility === 'MyOwnPrivate' || visibility === 'private') {
                newVisibility = setMyOwnPrivateVisibility();

            } else if(visibility=='MyOwnPublic' || visibility === 'public')
            {
                newVisibility = setMyOwnPublicVisibility();
            }else{
                (async ()=>{
                    ...
                    let baseUrl = "<?php echo $iot_directory_api?>"
                    let url = baseUrl + `/device.php?action=get_device&id=${deviceid}&contextbroker=${contextbroker}&nodered=true`;
                    $.ajax({
                        url: url,
                        method: 'GET',
                        async: true,
                        ...
                        success: function(data) {
                            if (data.status === 'ok') {
                                let v = data.content.visibility;
                                let isPrivate = v === "private";
                        ...
                        error: function(xhr) {
                            ...
                            try {
                                ...
                                errorMessage = JSON.stringify(responseJson, null, 2);
                            } catch (e) {
                                errorMessage = xhr.responseText;
                            }
                        
                            console.error(`Network error: ${errorMessage} - code: ${xhr.status}`);
                        }
                    });
                    
                })()                
            }
            function setMyOwnPrivateVisibility(){
            ...
            function setMyOwnPublicVisibility(){
                $('#visID').css('color', '#f3cf58');
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data["status"] === 'ok') {
                            ...
                            //Naldi 08/07/2025 -> update dateObserved because reading devices uses iot-search
                            (async () => {
                                let res = await updateDeviceAfterDelegation(contextbroker, deviceid, devicetype);
                                if (!res)
                                    console.error("Failed to update after deleting delegation: device " + deviceid)
                                else
            ...
            $(document).on("click", "#newOwnershipConfirmBtn", function(event) {
                ...
                //TODO
                //"NALDI 09/07/2025 -> To be fineshed of fixing
                // possible solution:
                // A delegate READ_WRITE to B (UPDATE API FILEMANAGER) + dateObserved update
                // change ownership from A a B
                // whne B open his Filemanager interface is necessary to find a way to let B understand that he must update file device (dateObserved) to complete change ownership operation
                ...
                success: function(data) {
                    ...
                    if (data["status"] === 'ok') {
                        ...
                        for (var i = 0; i < delegations.length; i++) {
                            if ((delegations[i].userDelegated !== "ANONYMOUS") && (delegations[i].userDelegated !== null) && (delegations[i].userDelegated !== undefined)) {
                                ...
                                success: function(data) {
                                    if (data["status"] === 'ok') {
                                        ...
                                        (async () => {
                                            let res = await updateDeviceAfterDelegation(contextbroker, deviceid, devicetype);
                                            if (!res)
                                                console.error("Failed to update after adding delegation: device " + deviceid)
                                            else
                                                console.log("Update successfully")
                                        })()
                                    } else {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data["status"] === 'ok') {
                            ...
                            var toaddK = $('#newDelegationOrganization').val();
        ...
        //Naldi 08/07/2025 -> update dateObserved of the device after delegation
        async function getToken() {
            try {
                let response = await fetch("../controllers/getAccessToken.php");
                if (!response.ok) {
                    let errorMessage = '';
                    try {
                    ...
                    throw new Error(`Network error: ${errorMessage} - code: ${response.status}`);
                }
        ...
        async function updateDeviceAfterDelegation(contextbroker, deviceid, devicetype) {
            let token = await getToken();
            
            if (token === undefined)
                return;
            //update dateObserved
            let baseUrl = "<?php echo $patch_url_filemanager?>"
            let url = baseUrl + `${contextbroker}/v2/entities/${deviceid}/attrs?elementid=${deviceid}&type=${devicetype}`;
            try {
