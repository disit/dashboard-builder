...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES["jsonFile"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (is_null($data)) {
            ...
            if (is_null($data['Widget']) || sizeof($data['Widget'])>1) {
                eventLog('Import Error not VALID Widget JSON.');
                //mysqli_rollback($link);
                echo "Import Error: not valid Widget JSON.";
                exit();
        ...
        $widgetData['creator'] = $userLog;
        //$widgetData['organizations'] = $org;
        ...
        //{
            ...
            //if($callResultArray['elementId']){
                //$dashboardId = mysqli_insert_id($link); // uguale alla variabile $newId?
                ...
                if($result3)
                {
                        ...
                        if (sizeof($row4)>1) {
                            ...
                            switch ($row4['type_w']) {
                                case 'widgetSce':
                                    ...
                                    $clonedWidgetName = preg_replace("/_\d+\_/", "_" . $dashboardId . "_", $clonedWidgetName);
                                    break;
                                ...
                                default:
                                    $clonedWidgetName = $row4['id_metric'] . '_' . $dashboardId . '_' . $row4['type_w'] . $clonedWidgetId;
                            ...
                            $queryMaxRow = "SELECT MAX(n_row + size_rows) AS maxRow FROM Dashboard.Config_widget_dashboard WHERE id_dashboard = '$dashboardId'";
                            $resultMaxRow = mysqli_query($link, $queryMaxRow);

                            if ($resultMaxRow) {
                                $rowMaxr = mysqli_fetch_assoc($resultMaxRow);
                                if ($rowMaxr['maxRow'] == null) {
                                    $firstFreeRow = 1;
                                } else {
                                    $firstFreeRow = $rowMaxr['maxRow'];
                                }
                                ...
                                if ($count == 0) {
                                    ...
                                    if (($key == "parameters" && $row4['type_w'] == "widgetMap") || ($key == "parameters" && $row4['type_w'] == "widget3DMapDeck") || ($key == "styleParameters" && $row4['type_w'] == "widgetTable")) {
                                        ...
                                        if ($key == "title_w") {
                                            ...
                                            $newQueryValues = $newQueryValues . ", " . $firstFreeRow;
                                        } else if ($key == "n_column") {
                                            $newQueryValues = $newQueryValues . ", " . "1";
                                        } else {
                                            $newQueryValues = $newQueryValues . ", " . returnManagedStringForDb($value);
                                        }
                                    }
                    ...
                    //         $clonedLogo = "../img/dashLogos/dashboard" . $dashboardId . "/" . $sourceDashLogoFilename;
                    ...
                    //      if (is_dir($uploadFolder)) {
                    //          $clonedScr = "../img/dashScr/dashboard" . $dashboardId . "/lastDashboardScr.png";
