...
if (empty($accessToken)) {
	$accessToken = isset($input['accessToken']) ? $input['accessToken'] : '';
		...
		echo json_encode(["error" => "Access Token not found in session or body of request"]);
		exit();}
}
...
function isAdmin($accessToken) {
	return true;
	#now uses checkSession("RootAdmin");
...
$resultwhosuser = isAdmin($accessToken);


if (!$resultwhosuser) {
	
	http_response_code(400);
    echo json_encode(["error" => "User doesn't seem to be userrootadmin"]);
    exit();
}
...
switch ($choice){
	...
	case "editApi":	
		...
		if ($stmt) {
			...
			if (mysqli_stmt_affected_rows($stmt) > 0) {
				echo json_encode(["results" => "status set to " . $settingStatus . "; then ".$response]);
			}
		} else {
			http_response_code(500);
			echo json_encode(["error" => "Query preparation failed: ". mysqli_error($link)]);
	...
	case "makeApi":
		...
		$sql = "";
		if ($selectAPIkind == "ClearMLStable" || $selectAPIkind == "ClearMLSporadic") {
			...
			$response = curl_exec($curl_plugin_create);
			$http_code_plugin_create = curl_getinfo($curl_plugin_create, CURLINFO_HTTP_CODE);
			
			if ($http_code_plugin_create !== 201) {
				http_response_code(500);
				echo json_encode(["error" => "Could not create create plugin in Kong. Expect inconsistencies: ".$response]);
				curl_close($curl_plugin_create);
				exit();
			}
			curl_close($curl_plugin_create);
			$extraparam = "{'ClearMLValue':'".$extraparam."','KongPluginID':'".$json_data['id']."'}";
