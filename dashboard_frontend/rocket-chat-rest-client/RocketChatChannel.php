...
class Channel extends Client {
	...
	*/
	public function info() {
        ...
        public function infoUserChannel($name,$id) {
		$response = Request::get( $this->api . 'channels.counters?roomName=' . $name .'&userId='.$id)->send();
		...
		$response = Request::get( $this->api . 'channels.info?roomId=' . $id )->send();

		if( $response->code == 200 && isset($response->body->success) && $response->body->success == true ) {
			$id = $response->body->channel->_id;
			return $response->body;
		} else {
			echo( $response->body->error . "\n" );
			return false;
	...
	*/
	public function findMembers($id){
		$response = Request::post( $this->api . 'channels.members' )
	...
	*/
	public function archive($idGroup){
		$response = Request::post( $this->api . 'channels.archive' )
			->body(array('roomId' => $idGroup))
			->send();
	...
	* Rename the channel
        * @Luciano Alessandro Ipsaro Palesi
	*/
	public function rename($idGroup,$newName){
		$response = Request::post( $this->api . 'channels.rename' )
			->body(array('roomId' => $idGroup, 'name' => $newName))
			->send();
	...
	*/
	 ...
	 * Adds user to channel.
	 */
	public function inviteId( $userId,$id ) {

		$response = Request::post( $this->api . 'channels.invite' )
			->body(array('roomId' => $id, 'userId' => $userId))
			->send();
                 if( $response->code == 200 && isset($response->body->success) && $response->body->success == true ) {
			return true;
		} else {
	 ...
	 * Adds owner to the channel.
	 */
	public function addOwnerId( $idRoom,$idUser ) {
                
            $response = Request::post( $this->api . 'channels.addOwner' )
			->body(array('roomId' => $idRoom, 'userId' => $idUser))
			->send();
	...
	public function removeOwner( $user ) {
        ...
        public function setUrl($id,$text){
		$response = Request::post( $this->api . 'channels.setAnnouncement' )
			->body(array('roomId' => $id , 'announcement' => $text))
			->send();
