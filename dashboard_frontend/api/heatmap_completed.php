...
if (isset($_POST["accessToken"])) {
    ...
    if (isset($_POST["heatmap_name"]) && isset($_POST["metric_name"]) && isset($_POST["time"])) {
        ...
        if ($response_get === false) {
            ...
            die(json_encode([
                "responseState" => "ko",
                "status" => "error",
                "message" => "Errore cURL durante invio messaggio complete: " . $error_get
            ]));
            ...
            if ($api_status_code !== 200) {
                ...
                die(json_encode([
                    "responseState" => "ko",
                    "status" => "error",
                    "message" => "Errore durante invio messaggio complete. Codice " . $api_status_code,
                    "response" => $response_get
                ]));
            }
        }

        logDebug(">>>>>>>>>>>> Heatmap complete message sent \n");
        ...
        $info_heatmap["responseState"] = "Required parameter missing";
        echo json_encode($info_heatmap);
        die();
    }
