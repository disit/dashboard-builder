...
<script type='text/javascript'>
    ...
    $(document).ready(function <?= $_REQUEST['name_w'] ?>(firstLoad)
    {
        ...
        $('#<?= $_REQUEST['name_w'] ?>_content').on('slid.bs.carousel', function (ev)
        {
            ...
            switch(id)
            {
                ...
                case "<?= $_REQUEST['name_w'] ?>_meteo":
                    ...
                    $('#<?= $_REQUEST['name_w'] ?>_meteo div.pcLegendaNessuno span').html("minimo");
        ...
        $("#<?= $_REQUEST['name_w'] ?>").on('updateFrequency', function(event){
            ...
            {
                var ref = "#ProtezioneCivileFirenze_" + idDash + "_widgetProtezioneCivileFirenze" + idWidget + "_countdownDiv";
                $(ref).text(counter);
                counter--;
                if (counter > 60)
                {
