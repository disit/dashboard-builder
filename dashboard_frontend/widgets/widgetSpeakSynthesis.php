...
<script type='text/javascript'>
                        ...
                        else
                        {
                            switch(metricType)
                            {
                                ...
                                case "Testuale":
                                   ...
                                   value="<input type=\"button\" value=\Play\ onclick=\"speak('"+JSON.parse(metricData.data[0].commit.author.value_text).text+"',"+JSON.parse(metricData.data[0].commit.author.value_text).pitch_value+","+JSON.parse(metricData.data[0].commit.author.value_text).rate_value+")\">"
                                   value="<script> speak('"+JSON.parse(metricData.data[0].commit.author.value_text).text+"',"+JSON.parse(metricData.data[0].commit.author.value_text).pitch_value+","+JSON.parse(metricData.data[0].commit.author.value_text).rate_value+");</\script>";
                                   break;
                            }
                        }
    ...
    var newWsValue ="<script> speak('"+JSON.parse(msgObj.newValue).text+"',"+JSON.parse(msgObj.newValue).pitch_value+","+JSON.parse(msgObj.newValue).rate_value+");</\script>"; 
 ...
 var synth = window.speechSynthesis;
	...
	function speak(text,pitch_value,rate_value){
		if (synth.speaking) {
		...
		if (text !== '') {
            ...
            if (pitch_value && rate_value) {
                ...
                value = "<input type=\"button\" value=\Play\ onclick=\"speak('" + text + "'," + pitch_value + "," + rate_value + ")\">"
                $("#<?= str_replace('.', '_', str_replace('-', '_', $_REQUEST['name_w'])) ?>_value span").html(value);
            }
	    }
	}
