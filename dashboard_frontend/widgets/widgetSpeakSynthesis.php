        ...
        function populateWidget()
        {
            if (metricData !== null)
            {
                if (metricData.data[0] !== 'undefined')
                {
				    ...
				    if (!IsJsonString(metricData.data[0].commit.author.value_text)) {
                                       metricData.data[0].commit.author.value_text = '{"text":"' + metricData.data[0].commit.author.value_text + '","pitch_value":1,"rate_value":1}' ;
                                    }
                                    value = "<input type=\"button\" value=\Play\ onclick=\"speak('" + JSON.parse(metricData.data[0].commit.author.value_text).text + "'," + JSON.parse(metricData.data[0].commit.author.value_text).pitch_value + "," + JSON.parse(metricData.data[0].commit.author.value_text).rate_value + ",'" + JSON.parse(metricData.data[0].commit.author.value_text).lang + "')\">"
                                    value = "<script> speak('" + JSON.parse(metricData.data[0].commit.author.value_text).text + "'," + JSON.parse(metricData.data[0].commit.author.value_text).pitch_value + "," + JSON.parse(metricData.data[0].commit.author.value_text).rate_value + ",'" + JSON.parse(metricData.data[0].commit.author.value_text).lang + "');</\script>";
                                    break;
                            }
                        }
        ...
        $.ajax({
            ...
            success: function (widgetData)
            {
                ...
                } else
                {
					...
					if (!isNaN(parseFloat(data[0].value)))
                                        {
                                            originalMetricType = 'float';
                                        } else
                                        {
                                            // if (parseInt(data[0].value) !== 'NaN')
					    if (!isNaN(parseInt(data[0].value)))
                                            {
                                                originalMetricType = 'integer';
                                            } else
                                            {
...
?>
            ...
            switch (msgObj.msgType)
            {
                case "newNRMetricData":
                    if (encodeURIComponent(msgObj.metricName) === encodeURIComponent(metricName))
                    {
                        ...
                        //var newWsValue ="<input type=\"button\" value=\NewMessage\ onclick=\"speak('"+msgObj.newValue+"',"+1+","+1+")\">"
                        //var newWsValue ="<script>setTimeout(function(){ speak('"+msgObj.newValue+"',"+1+","+1+");alert("Hello"); }, 5);<\script>"
                        var newWsValue = "<input type=\"button\" value=\NewMessage\ onclick=\"alert(555)\">"
                        var newWsValue = "<script> speak('" + JSON.parse(msgObj.newValue).text + "'," + JSON.parse(msgObj.newValue).pitch_value + "," + JSON.parse(msgObj.newValue).rate_value + ",'" + JSON.parse(msgObj.newValue).lang + "');</\script>";
    ...
    function speak(text, pitch_value, rate_value, lang) {
        if (synth.speaking) {
        ...
        if (text !== '') {
            ...
            for (i = 0; i < voices.length; i++) {
                if ((voices[i].name).includes(nameLang) && (voices[i].lang) === natLang) {
                    voiceSel = voices[i];
                    break;
                }
            }
	...
	if (pitch_value && rate_value) {
            utterThis.voice = voiceSel;
            utterThis.pitch = pitch_value;
            utterThis.rate = rate_value;
            synth.speak(utterThis);
            value = "<input type=\"button\" value=\Play\ onclick=\"speak('" + text + "'," + pitch_value + "," + rate_value + ",'" + lang + "')\">"
            $("#<?= str_replace('.', '_', str_replace('-', '_', $_REQUEST['name_w'])) ?>_value span").html(value);
	}    
        }
    }
