...
function showAddUserModal()
{
    ...
    $("#usersTable2 tbody tr").each(function(i){
...
//Funzione per aggiunta nuovo utente al sistema
        ...
        success: function (data) 
        {
            switch(data)
            {
                ...
                case '1':
                    ...
                    $("#addUserModalBody").show();
                    $("#addUserModalFooter").show();
                    setTimeout(addUserTimeout, 2000);
                    break;
...
//Funzione di salvataggio dei dati degli utenti
/*function saveUsersData()
{
    var username, usersJson, singleUserUpdated, timeout, reloadPage = null;
    $("#saveUsersDataModal div.modal-body").html("");
    $("#saveDataUsersCancelBtn").hide();
    $("#saveDataUsersConfirmBtn").hide();
    $("#saveUsersDataModal div.modal-body").append('<div id="saveUsersDataModalInnerDiv1" class="modalBodyInnerDiv"><h5>Users data update in progress, please wait</h5></div>');
    $("#saveUsersDataModal div.modal-body").append('<div id="saveUsersDataModalInnerDiv2" class="modalBodyInnerDiv"><i class="fa fa-circle-o-notch fa-spin" style="font-size:42px"></i></div>');
    
    usersJson = [];
    
    //Costruzione del sottoinsieme di utenti di cui aggiornare i dati
      $("#usersTable2 tr").each(function(i){
          if(i > 0)
          {
              if($(this).attr("data-edited") === 'true')
              {
                  var usertypeNum = null;
                  if($(this).find("td").eq(5).find("a").html() === 'End user')
                  {
                      usertypeNum = 0;
                  }
                  else if($(this).find("td").eq(5).find("a").html() === 'Admin')
                  {
                      usertypeNum = 1;
                  }
                  else if($(this).find("td").eq(5).find("a").html() === 'Super admin')
                  {
                      usertypeNum = 2;
                  }

                  var statusNum = null;
                  if($(this).find("td").eq(8).find("a").html() === 'Active')
                  {
                      statusNum = 1;
                  }
                  else if($(this).find("td").eq(8).find("a").html() === 'Not active')
                  {
                      statusNum = 0;
                  }
                  ...
                  singleUserUpdated = {
                     ...
                     password: $(this).find("td").eq(1).find("a").html(),
                     name: $(this).find("td").eq(2).find("a").html(),
                     surname: $(this).find("td").eq(3).find("a").html(),
                     organization: $(this).find("td").eq(4).find("a").html(),
                     usertype: usertypeNum,
                     email: $(this).find("td").eq(6).find("a").html(),
                     status: statusNum
                  };
                  usersJson.push(singleUserUpdated);
              }
          }
      });
    
    //Chiamata API di salvataggio dei dati utente
    $.ajax({
        url: "saveUsersData.php",
        ...
        success: function (data) 
        {
            if(data === '0')
            {
                $("#saveUsersDataModalInnerDiv1").html('User data update failed, please try again');
                $("#saveUsersDataModalInnerDiv2").html('<i class="fa fa-frown-o" style="font-size:42px"></i>');
                timeout = 2000;
                reloadPage = true;
            }
            else if( data === '1')
            {
                $("#saveUsersDataModalInnerDiv1").html('User data updated successfully');
                $("#saveUsersDataModalInnerDiv2").html('<i class="fa fa-check" style="font-size:42px"></i>');
                timeout = 1200;
                reloadPage = false;
                if(admin === 1)
                {
                    $("#usersTable1 a").removeClass("editable-unsaved");
                }
                else if(admin === 2)
                {
                    $("#usersTable2 a").removeClass("editable-unsaved");
                }
            }
        },
        error: function (data) 
    ...
    setTimeout(function()
    {
        ...
        {
            $("#usersTable1 tr").each(function(i)
            {
                if(i > 0)
                {
                    if($(this).attr("data-edited") === 'true')
            ...
            $("#saveDataUsersCancelBtn").show();
            $("#saveDataUsersConfirmBtn").show();
            $("#saveUsersDataModal div.modal-body").html('<div class="modalBodyInnerDiv">Do you want to confirm changes made to user data?</div>');
        }
    }, timeout);
}*/
...
function deleteUser()
{
    ...
    $("#deleteUserModal div.modal-body").append('<div id="deleteUserModalInnerDiv2" class="modalBodyInnerDiv"><i class="fa fa-circle-o-notch fa-spin" style="font-size:42px"></i></div>');
