<?php 
   ...
   //     eventLog("Returned the following ERROR in customTheme.php for dashId = ".$dashId.": ".$dashId." is not an integer as expected. USER = " . $_SESSION['loggedUsername'] . ". Exit from script.");
   //     eventLog("Returned the following ERROR in customTheme.php for dashId = ".$dashId.": ".$dashId." is not an integer as expected. Exit from script.");
        eventLog("Returned the following ERROR in customTheme.php for dashId = ".$dashId.": ".$dashId." is not an integer as expected. Exit from script. HTTP REFERER: ".$_SERVER['HTTP_REFERER'] . "; Request URI: " . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] . ". Request Method: " . $_SERVER['REQUEST_METHOD']);
        exit();
   };
    ...
    if($queryResult) 
    {
        ...
        else
        {
           ...
           if($queryResult->num_rows > 0) 
           {
                ...
                $_SESSION['colorsArray'] = [
                    "header_bg" => $row['color_header'], //Sfondo header
                    "widgetArea_bg" => $row['color_background'], //Sfondo area widget
                    "body_bg" => $row['external_frame_color'], //Sfondo body
                    "header_color" => $row['headerFontColor'], //Colore header
                    "grid" => $row['gridColor']
                ]; 
...
<html lang="en">
    ...
    <link href="../css/themes/customTheme/S4C-customTheme-dashboard-view.css?v=<?php echo time();?>" rel="stylesheet">
    <link rel="stylesheet" href="../css/themes/customTheme/style_widgets.css?v=<?php echo time();?>" type="text/css" />
    <style>
        :root{
            --header-bg: <?php echo $_SESSION['colorsArray']["header_bg"] ?>;
            --widgetArea-bg: <?php echo $_SESSION['colorsArray']["widgetArea_bg"] ?>;
            --body-bg: <?php echo $_SESSION['colorsArray']["body_bg"] ?>;
            --header-color: <?php echo $_SESSION['colorsArray']["header_color"] ?>;
        }
    </style>
    <!-- Material icons -->
  ...
  <!--  <script src="https://rawgithub.com/miladjafary/highcharts-plugins/master/js/jalali.js"></script>    -->
   ...
   <script src="https://www.snap4city.org/synoptics/socket.io/socket.io.js"></script>
    ...
    <script type='text/javascript'>
        var colors = <?php echo json_encode($_SESSION['colorsArray']) ?>;
        console.log("TEST", Object.entries(colors));
         ...
         //  $('.linkgen').click(function(event){
            ...
            function loadDashboard(dashboardParams, dashboardWidgets)
            {
               ...
               else
               {
                ...
                console.log("WIDGETS TEST", dashboardWidgets)
                for(var i = 0; i < dashboardWidgets.length; i++)
                {
                    ...
                    dashboardWidgets[i].hostFile = 'customTheme';
                    $("li#" + dashboardWidgets[i]['name_w']).css('border', '1px solid ' + dashboardWidgets[i].borderColor);
                    $("div#" + dashboardWidgets[i]['name_w']+"_header").css('background-color', dashboardWidgets[i].frame_color_w);
                ...
                // setTimeout(async () => {
                //     dashboardWidgets.forEach(x => {
                //         let name = x.name_w
                //         var titleDiv = $("#"+name+"_titleDiv")
                //         let tmp = `<span class="widgetTitleSpan"> ${titleDiv.text()} </span>`
                //         titleDiv.html(tmp)
                //     })
                // }, dashboardWidgets.length * 200);
                        ...
                        switch(response.visibility)
                        {
                            ...
                            case 'author': case 'restrict':    
                                ...
                                switch(response.detail)
                                {
                                    ...
                                    default:
                                        //$("#dashboardViewMainContainer").hide();
                                        $("#dashboardViewHeaderContainer").css("display", "contents");
        ...
        <nav id="dashboardViewHeaderContainer" class="navbar navbar-fixed-top" role="navigation">
                    ...
                    <i id="restorescreenButton" class="fa fa-window-restore"></i>
                            ...
                            if($r) {
                                while ($row = mysqli_fetch_assoc($r)) {
                                    ...
                                    $orgMenuIconHidden = '<i id="orgMenuButton" class="fa fa-caret-down" style="display:none"></i>';
                                    $orgMenuIconShown = '<i id="orgMenuButton" class="fa fa-bars menu-btn"></i>';
            ...
            <nav id="s4c-sidebar" class="sidebar-menu" role="navigation">
              <div class="sidebar-container">
                    <ul class="main-menu">
                           <!-- <li class="s4chome"><a href="#"><span class="material-icons-round menu-icon">home</span><span class="menu-item">SNAP4CITY.ORG</span></a></li> -->
                        <?php
                        ...
                        if ($orgMenuVisible == 'yes') {
                            eventLog("Menu visible for dashboard id = " . $dashId);
                            ...
                            //Ha bisogno di $dashId
                            include "customLinks-side.php";
                                        ...
                                        while ($row2 = mysqli_fetch_assoc($r2)) {
                                            ...
                                            if ($allowedOrgs2 == '*' || strpos($allowedOrgs2, "'" . $organizationSql) !== false || $_SESSION['loggedRole'] == 'RootAdmin') {
                                                if ($externalApp2 == 'yes') {
                                                    if ($openMode2 == 'newTab') {
                                                        if ($_REQUEST['fromSubmenu'] == false || $_REQUEST['fromSubmenu'] != $linkId) {
                                                            $newItem = '<div data-selected="false">' .
                                                                '<div class="orgMenuSubItemCnt feat-show sub-menu" data-fatherMenuIdDiv="' . $linkId . '" style="display: none">' .
              ...
              <script>
                $('.menu-btn').click(function(){
                 ...
                 $('.feat-btn').click(function(){
                    $('nav#s4c-sidebar ul .feat-show').toggleClass("show");
                    $('nav#s4c-sidebar ul .first').toggleClass("rotate");
                 });
                 $('.serv-btn').click(function(){
                    $('nav#s4c-sidebar ul .serv-show').toggleClass("show1");
                    $('nav#s4c-sidebar ul .second').toggleClass("rotate");
                 });
                   $('nav#s4c-sidebar ul li').click(function(){
                    $(this).addClass("active").siblings().removeClass("active");
                 });
                    $(document).on("click", ".sublinkFather", (evt) => {
                        let currTarget = evt.currentTarget
                        let fatherId = currTarget.getAttribute("data-linkid")
                        if(fatherId != ""){
                            $(`.sublink[data-fathermenuiddiv=${fatherId}]`).toggle()
                            $(currTarget).find(".submenuIndicator").toggleClass("fa-caret-right").toggleClass("fa-caret-down")
                        }
                    })
                </script>
                <link rel="stylesheet" href="../css/customLinks-side.css"> 
              <!-- END SIDEBAR NAV -->
